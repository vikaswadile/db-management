...
while [[ ! $country =~ ^(kenya|rwanda)$ ]]; do
    read -p "Enter a country (kenya/rwanda): " country
    echo "Enter country name either kenya or rwanda"
done

while [[ ! $env_name =~ ^(prod|stage)$ ]]; do
    read -p "Please enter environment (prod/stage) : " env_name
    echo "Enter environment either prod or stage"
done

while [[ -z $reason ]]; do
    read -p "Please enter reference Jira ticket or mention reason : " reason
    echo "please enter reason, do not keep it blank"
done
